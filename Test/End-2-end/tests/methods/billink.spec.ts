...
const testCases = [
  {status: 'authorized', orderStatus: 'Processing', title: 'Validate the submission of an order with Billink as payment method and payment mark as "Authorized"'},
  {status: 'failed', orderStatus: 'Canceled', title: 'Validate the submission of an order with Billink as payment method and payment mark as "Failed"'},
  {status: 'expired', orderStatus: 'Canceled', title: 'Validate the submission of an order with Billink as payment method and payment mark as "Expired"'},
  {status: 'canceled', orderStatus: 'Canceled', title: 'Validate the submission of an order with Billink as payment method and payment mark as "Canceled"'},
];

for (const testCase of testCases) {
  test(testCase.title, async ({ page }) => {
    test.skip(!process.env.mollie_available_methods.includes('billink'), 'Skipping test as Billink is not available');
    await visitCheckoutPayment.visit(page);

    await checkoutPaymentPage.selectPaymentMethod(page, 'Billink');
