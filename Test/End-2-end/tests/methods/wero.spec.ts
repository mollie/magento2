...
const testCases = [
  {status: 'paid', orderStatus: 'Processing', title: 'Validate the submission of an order with Wero as payment method and payment mark as "Paid"'},
  {status: 'failed', orderStatus: 'Canceled', title: 'Validate the submission of an order with Wero as payment method and payment mark as "Failed"'},
  {status: 'expired', orderStatus: 'Canceled', title: 'Validate the submission of an order with Wero as payment method and payment mark as "Expired"'},
  {status: 'canceled', orderStatus: 'Canceled', title: 'Validate the submission of an order with Wero as payment method and payment mark as "Canceled"'},
];

for (const testCase of testCases) {
  test(testCase.title, async ({ page }) => {
    test.skip(!process.env.mollie_available_methods.includes('wero'), 'Skipping test as Wero is not available');
    await visitCheckoutPayment.visit(page);

    await checkoutPaymentPage.selectPaymentMethodByCode(page, 'mollie_methods_wero');
