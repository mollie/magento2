...
for (const testCase of testCases) {
  test(testCase.title, async ({ page }) => {
    test.skip(!process.env.mollie_available_methods.includes('paybybank'), 'Skipping test as Paybybank is not available');
    await visitCheckoutPayment.visit(page);

    await page.route('**/mollie/checkout/redirect/paymentToken/*', (route) => route.continue());
    await checkoutPaymentPage.selectPaymentMethod(page, 'Pay by Bank');
