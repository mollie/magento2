...
- Removal of Klarna Pay Later, Klarna Pay Now & Klarna Slice it in favor of the general Klarna method.
  - For old orders that need to be refunded, please use the Mollie Dashboard.

- The manual capture option is now per-method. If you are using this option, please review the settings.
...
### Analytics addon merged into the main module

The standalone `mollie/magento2-analytics` package has been merged into the main module. Existing rows in `mollie_analytics_analytics` are migrated to a new `mollie_payment_tracking` table on `setup:upgrade`; the legacy table is dropped afterwards.

The wiring also gained support for capturing arbitrary tracking cookies. A new admin field at **Stores → Configuration → Mollie → Developer Settings → Advanced → Tracking cookies** lets merchants list any cookie/alias pair they want to forward through the Mollie redirect — raw cookie values are forwarded as-is. See issue [magmodules/mollie-magento2#1062](https://github.com/magmodules/mollie-magento2/issues/1062).

If you previously relied on the GA client-id extraction (the `GA1.2.` prefix being stripped server-side), you need to handle that in your success-page JavaScript:
...
The legacy `Mollie\Analytics\…` namespace, the `mollie_analytics_analytics` table, and its `client_id` column are removed without backwards-compatibility shims. `composer.json` ships `"replace": { "mollie/magento2-analytics": "*" }` so the addon is removed automatically on upgrade.
