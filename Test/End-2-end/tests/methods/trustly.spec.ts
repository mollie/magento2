...
for (const testCase of testCases) {
  test(testCase.title, async ({ page }) => {
    test.skip(!process.env.mollie_available_methods.includes('trustly'), 'Skipping test as Trustly is not available');
    await visitCheckoutPayment.visit(page);

    await checkoutPaymentPage.selectPaymentMethod(page, 'Trustly');
    await checkoutPaymentPage.placeOrder(page);

    await mollieHostedPaymentPage.fillCustomerInformationIfRequired(page);
