...
for (const testCase of testCases) {
  test(testCase.title, async ({ page }) => {
    test.skip(!process.env.mollie_available_methods.includes('blik'), 'Skipping test as Blik is not available');

    await visitCheckoutPayment.changeCurrencyTo(page, 'PLN');
    await visitCheckoutPayment.visit(page);

    await checkoutPaymentPage.selectPaymentMethod(page, 'blik');
