...
for (const testCase of testCases) {
  test(testCase.title, async ({ page }) => {
    test.skip(!process.env.mollie_available_methods.includes('payconiq'), 'Skipping test as Payconiq is not available');
    await visitCheckoutPayment.visit(page, 'BE');

    await checkoutPaymentPage.selectPaymentMethod(page, 'Payconiq');
