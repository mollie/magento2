...
const testCases = [
  {status: 'paid', orderStatus: 'Processing', title: '[C4231942] Validate the submission of an order with Multibanco as payment method and payment mark as "Paid"'},
  {status: 'failed', orderStatus: 'Canceled', title: '[C4231943] Validate the submission of an order with Multibanco as payment method and payment mark as "Failed" '},
  {status: 'expired', orderStatus: 'Canceled', title: '[C4231944] Validate the submission of an order with Multibanco as payment method and payment mark as "Expired"'},
  {status: 'canceled', orderStatus: 'Canceled', title: '[C4231945] Validate the submission of an order with Multibanco as payment method and payment mark as "Cancelled" '},
];

for (const testCase of testCases) {
  test(testCase.title, async ({ page }) => {
    test.skip(!process.env.mollie_available_methods.includes('multibanco'), 'Skipping test as Multibanco is not available');
    await visitCheckoutPayment.visit(page);

    await checkoutPaymentPage.selectPaymentMethod(page, 'Multibanco');
